...
# Поля author' и 'created_at сериализатор не меняет -
# определяются автоматически.
# Валидация отсутствия поля комментария в запросе
# или наличия пустой строки комментария.
...
class CommentSerializer(serializers.ModelSerializer):
    ...
    # Метод сериализатора. Валидация длины текста комментария -
    # не больше 300 символов.
    def validate_text(self, value):
        if len(value) > 300:
            raise serializers.ValidationError(
                'Комментарий должен быть меньше 300 символов.'
                )
        return value

# Сериализатор для Post. В API выводит поля id, text, image, created_at и
# добавляет поля:
...
class PostSerializer(serializers.ModelSerializer):
    ...
    # Метод сериализатора. Валидация длины текста поста -
    # не больше 3000 символов.
    def validate_text(self, value):
        if len(value) > 3000:
            raise serializers.ValidationError(
                'Длина поста не должна превышать 3000 символов.'
                )
        return value
    ...
    # Метод сериализатора.
    # Возвращает количество активных лайков для текущего поста.
